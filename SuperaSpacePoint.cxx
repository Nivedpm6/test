...
namespace larcv {
  ...
  void SuperaSpacePoint::configure(const PSet& cfg)
  {
    ...
    _optional_hit_label = cfg.get<std::string>("OptionalHitProducer","");
  ...
  bool SuperaSpacePoint::process(IOManager& mgr)
  {
    ...
    for (auto const& label : _producer_labels) {
      ...
      std::string hit_label = _optional_hit_label.empty() ? label : _optional_hit_label;
      size_t n_hits = ev->getValidHandle<std::vector<recob::Hit>>(hit_label)->size();
